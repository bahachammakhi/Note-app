...
class App extends Component {
  ...
  addNote(note, note1, note2) {
    firebase
      ...
      .child("notes")
      .push()
      .set({ noteTitle: note, notePrag: note1, noteTimeAdded: note2 });
  }
  addTodoItem = (content, time) => {
    firebase
      ...
      .child("todolist")
      .push()
      .set({ todoContent: content, todoTimeAdded: time });
  };
  removetodo = TodoId => {
  ...
  EditNote = (NoteId, title, content, time) => {
    firebase
      ...
      .child("notes")
      .child(NoteId)
      .set({ noteTitle: title, notePrag: content, noteTimeAdded: time });
  };
  removeNote(noteId) {
  ...
  render() {
    ...
    //signin page
    const loading = <div className="spinner-border" />;

    const AfficheNote = this.state.loadingnote ? (
      <div className="card-columns notemobile"> {noteitem}</div>
    ) : (
      loading
    );

    const AfficheTodo = this.state.loadingtodo ? (
      <div className="list-group list-group-flush"> {todoItem}</div>
    ) : (
      loading
    );
    //main of the page
    const main = (
      <div className="body">
        <div className="d-block d-sm-none">
          <NavBar
            note={this.handleNoteComponent}
            todo={this.handleTodoComponent}
        ...
        <UnmountClosed isOpened={this.state.noteO}>
          <div className="notePage" className={this.state.main}>
            <div className="container">
              <div className="row ">
                <div className=" w-25 col align-self-center">
                  {" "}
                  <SimpleModal
                    handleCloseNoteform={this.handleCloseNoteform}
        ...
        <UnmountClosed isOpened={this.state.todoOpen}>
          <div className="row " className={this.state.main}>
            <div className="">
              ...
              <div className=" todoform mt-3">
                {" "}
                <TodoForm addTodoItem={this.addTodoItem} />{" "}
              </div>
              <div className="todolist margin-bottom ">{AfficheTodo}</div>
            </div>
          </div>
        </UnmountClosed>
      </div>
    ...
    return (
      <div>
        {this.state.isSignedIn ? (
          ...
          <div className="d-flex justify-content-center   mr-2 widthcard">
            {" "}
            <SignIn />
          </div>
        )}
